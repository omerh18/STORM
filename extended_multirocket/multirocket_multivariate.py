...
@njit("float32[:](float64[:,:,:],int32[:],int32[:],int32[:],int32[:],float32[:],int32[:,:],int32)",
      fastmath=True, parallel=False, cache=True)
def _fit_biases(X, num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, quantiles, indices, kernel_length):
    ...
    for dilation_index in range(num_dilations):
        ...
        for kernel_index in range(num_kernels):
            ...
            C_gamma = np.zeros((kernel_length, num_channels_this_combination, input_length), dtype=np.float32)
...
def _fit_dilations(input_length, num_features, max_dilations_per_kernel, kernel_length):
    ...
    #num_kernels = 84
    num_kernels = int(scipy.special.binom(kernel_length, 3))
...
def fit(X, num_features=10_000, max_dilations_per_kernel=32, kernel_length=9, max_channels_per_kernel=9, random_num_channels=True, channels_per_kernel=9):
    ...
    max_num_channels = min(num_channels, max_channels_per_kernel)
    ...
    if random_num_channels:
        ...
        num_channels_per_combination = np.full(num_combinations, min(channels_per_kernel, max_num_channels - 1))
    ...
    indices = np.array([_ for _ in combinations(np.arange(kernel_length), 3)], dtype = np.int32)
    biases = _fit_biases(X, num_channels_per_combination, channel_indices,
                         dilations, num_features_per_dilation, quantiles, indices, kernel_length)

    return num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, biases, indices, kernel_length


@njit("float32[:,:](float64[:,:,:],float64[:,:,:],Tuple((int32[:],int32[:],int32[:],int32[:],float32[:],int32[:,:],int32)),Tuple((int32[:],int32[:],int32[:],int32[:],float32[:],int32[:,:],int32)),int32,int32)",
      fastmath=True, parallel=True, cache=True)
def transform(X, X1, parameters, parameters1, n_features_per_kernel=4, calc_first_order_diff=1):
    num_examples, num_channels, input_length = X.shape

    num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, biases, indices, kernel_length  = parameters
    _, _, dilations1, num_features_per_dilation1, biases1, _, _ = parameters1
    ...
    if calc_first_order_diff:
        ...
        n_features_per_transform = np.int64(features.shape[1])
    ...
    for example_index in prange(num_examples):
        ...
        for dilation_index in range(num_dilations):
            ...
            C_gamma = np.zeros((kernel_length, num_channels, input_length), dtype=np.float32)
            C_gamma[kernel_length // 2] = G
        ...
        if not calc_first_order_diff: ##added
            continue
        ...
        for dilation_index in range(num_dilations1):
            ...
            C_gamma = np.zeros((kernel_length, num_channels, input_length - 1), dtype=np.float32)
            C_gamma[kernel_length // 2] = G1
